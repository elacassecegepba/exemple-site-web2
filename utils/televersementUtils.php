 ...
 * @param array   $typesAuthorises       Un tableau des types de fichiers authorisés.
 *                                       Ex. : ['image/png' => 'png', 'image/jpeg' => 'jpg']
 */
function preparerFichiersPourTeleversement($nomInput, $dossierTeleversement, $tailleMaximaleMB, $typesAuthorises)
{
	$infosValidation = validerFichiers($nomInput, $tailleMaximaleMB, $typesAuthorises);
...
function validerFichiers($nomInput, $tailleMaximaleMB, $typesAuthorises)
{
  ...
  for ($i=0; $i < sizeof($fichiers); $i++) {
    ...
    $validation = validerFichier($infosFichier, $tailleMaximaleMB, $typesAuthorises);
  ...
  if (!empty($erreurs)) {
    $resultat['erreur'] = $erreurs;
  }

  return $resultat;
}

function validerFichier($infosFichier, $tailleMaximaleMB, $typesAuthorises) {
